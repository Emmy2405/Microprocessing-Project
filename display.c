...
void printTime(uint16_t minutes, uint16_t seconds, uint16_t x, uint16_t y, uint16_t ForeColour, uint16_t BackColour);
...
void display_begin()
{
	...
	fillRectangle(0,0,SCREEN_WIDTH, SCREEN_HEIGHT, 0x001F);  // black out the screen
}
void ResetLow()
{
	GPIOA->ODR &= ~(1u << 3);
...
void clear()
{
	fillRectangle(0,0,SCREEN_WIDTH, SCREEN_HEIGHT, RGBToWord(0, 0, 0 ));  // black out the screen
}
...
void clear2()
{
	fillRectangle(0,0,12, 16,RGBToWord(47, 176, 255));  // black out the screen
}

void clearImage(int x, int y, int width, int height) 
{
    // Implement your logic to clear or hide the image at the specified position
    // This could involve filling the region with the background color or any other appropriate method.

    // For example, assuming you have a fillRectangle function:
    fillRectangle(x, y, width,  height, RGBToWord( 47, 176, 255));
}

void printTime(uint16_t minutes, uint16_t seconds, uint16_t x, uint16_t y, uint16_t ForeColour, uint16_t BackColour)
{
    char Buffer[6]; // Buffer to hold the time string
    Buffer[5] = 0; // Null-terminate the string

    // Convert minutes to string
    Buffer[0] = minutes / 10 + '0'; // Tens digit of minutes
    Buffer[1] = minutes % 10 + '0'; // Ones digit of minutes

    // Add ':' separator
    Buffer[2] = ':';

    // Convert seconds to string
    Buffer[3] = seconds / 10 + '0'; // Tens digit of seconds
    Buffer[4] = seconds % 10 + '0'; // Ones digit of seconds
